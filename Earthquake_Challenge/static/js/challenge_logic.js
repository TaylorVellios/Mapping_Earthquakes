...
d3.json('https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson').then(function(data) {
  ...
  L.geoJson(data, {
      ...
      onEachFeature: (feature, layer) => {
        ...
        layer.bindPopup(`<h2><u>Magnitude</u>: ${feature.properties.mag}</h2><hr><h3><u>Location</u>: ${feature.properties.place}<br><u>Time</u>: ${time}</h3>`)
      }
  }).addTo(majorquakes);
...
// Looping through our intervals to generate a label with a colored square for each interval.
  ...
  d3.json("https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json").then(i => {
    L.geoJson(i,{
      color: 'purple',
      weight: 2}).addTo(plates);
  });
  plates.addTo(map);
});